...
with overview_tab:
    ...
    if not latest_market_row.empty:
        metric_columns[3].metric(
            "Monthly leader",
            str(latest_market_row.iloc[0]["best_performing_etf"]),
            delta=format_pct(
                latest_market_row.iloc[0][
                    "best_performing_etf_return_pct"
                ]
            ),
            border=True,
        ...
        metric_columns[3].metric("Monthly leader", "—", border=True)

    if not best_long_term_row.empty:
        metric_columns[4].metric(
            "Best long-term ETF",
            str(best_long_term_row.iloc[0]["symbol"]),
            delta=format_pct(
                best_long_term_row.iloc[0]["total_return_pct"]
            ),
            border=True,
        )
    else:
    ...
    if not lowest_volatility_row.empty:
        metric_columns[5].metric(
            "Lowest volatility",
            str(lowest_volatility_row.iloc[0]["symbol"]),
            delta=format_pct(
                lowest_volatility_row.iloc[0][
                    "monthly_volatility_pct"
                ]
            ),
            delta_color="off",
    ...
    st.dataframe(
        ...
        hide_index=True,
        column_config={
            "symbol": st.column_config.TextColumn("ETF"),
...
with explorer_tab:
    ...
    st.subheader(f"{selected_symbol} ETF Explorer")
    ...
    if symbol_monthly.empty:
        ...
        with right_column:
            return_figure = px.bar(
                ...
                title=f"{selected_symbol} monthly returns",
                labels={
                    "month_start_date": "Month",
...
with market_tab:
    ...
    if market_history.empty:
        ...
        winners = (
            market_history["best_performing_etf"]
            .dropna()
            .astype(str)
            .value_counts()
            .rename_axis("symbol")
